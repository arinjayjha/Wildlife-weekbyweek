// Sample question data for demonstration purposes
const questions = {
    "1": [
        {
        ...
        },
        {
            ...
            "options": ["Fitness is environment-specific.", "Fitness is species-specific.", "Higher reproductive rate means higher fitness.", "Fitness should be measured across several generations."],
        ...
        {
            "question": "Which of these is not a characteristic of fitness?",
            "options": ["Fitness is environment-specific.", "Fitness is species-specific.", "Fitness works on traits such as size and speed.", "Fitness should be measured across several generations."],
    ...
    "4": [
        {
            ...
            "options": ["available energy decreases", "available energy increases", "available energy remains same", "available energy is zero everywhere"],
            "answer": "available energy decreases"
        },
        {
            "question": "Consider the food chain: Grass → Grasshopper → Frog → Snake → Hawk. In this food chain,",
            "options": ["hawk is producer", "hawk is consumer and carnivore", "hawk is consumer and herbivore", "hawk is decomposer"],
        ...
        {
            "question": "Consider the food chain: Grass → Grasshopper → Frog → Snake → Hawk. In this food chain,",
            "options": ["frog is producer", "frog is consumer and carnivore", "frog is consumer and herbivore", "frog is decomposer"],
        ...
        {
            "question": "Consider the food chain: Grass → Grasshopper → Frog → Snake → Hawk. In this food chain,",
            "options": ["more number of hawks than grasshoppers can be supported", "more number of grasshoppers than hawks can be supported", "equal number of hawks and grasshoppers can be supported", "none of these"],
    ...
    "6": [
        ...
        },
        {
            ...
            "options": ["Relative density + Relative frequency X Relative dominance",
    ...
    "8": [
        ...
        },
        {
            ...
            "options": ["Original forest → Dissection → Perforation → Fragmentation → Attrition",
    ...
    "9": [
        ...
        },
        {
            ...
            "options": ["high birth rate, low death rate to low birth rate, low death rate",
        ...
        },
        {
            ...
            "options": ["Population grows in geometric progression, food supply increases in arithmetic progression",
    ...
    "10": [
        ...
        {
            "question": "The government came up with a regulation that incandescent bulbs be replaced by LED bulbs to reduce electricity consumption and release of carbon dioxide from power plants. Such an action would be called",
        ...
        {
            "question": "The ability of a system to adjust to climate change to moderate potential damages, take advantage of opportunities, or cope with the consequences is a definition for",
        ...
        {
            "question": "Because of climate change, Mudumalai Tiger Reserve is suffering from frequent droughts. The management has built several artificial water holes for animals, and fills them up regularly with tankers. Such an action would be called",
            "options": ["adaptation", "mitigation", "deceleration", "maladaptation"],
            "answer": "adaptation"
        }
    ]
    ...
    "11": [
        ...
        },
        {
            ...
            "options": ["it is not increasing", "it is decreasing", "it is not causing any economic damage", "it is not causing excessive economic damage"],
        ...
        },
        {
            ...
            "options": ["it is increasing", "it is not decreasing", "it is causing some economic damage", "it is causing excessive economic damage"],
        ...
        {
            "question": "Which of these is correct?",
            "options": ["The maximum sustainable yield is near the beginning of the sigmoidal curve",
...
function loadWeek(week) {
    currentWeekQuestions = [...questions[week]];
    usedQuestions = [];
    questionCount = currentWeekQuestions.length;
    score = 0;
    updateStatus();
    showNextQuestion();
}
...
function updateStatus() {
    ...
    const remaining = currentWeekQuestions.length;
    const completed = questionCount - remaining;
    statusElement.innerText = `Questions Completed: ${completed} / ${questionCount}`;
}
...
function showNextQuestion() {
    ...
    const questionIndex = Math.floor(Math.random() * currentWeekQuestions.length);
    const questionData = currentWeekQuestions[questionIndex];
    currentWeekQuestions.splice(questionIndex, 1); // remove used question
        ...
        <div class="question">
            ...
            <div class="options">
                ${data.options.map((option, index) => `
                    <button onclick="checkAnswer(this, '${option}', '${data.answer}')">${option}</button>
                `).join('')}
            </div>
            <div id="status"></div>
        </div>
...
function checkAnswer(button, selectedAnswer, correctAnswer) {
    ...
    setTimeout(showNextQuestion, 1000); // Wait a moment before showing next question
}

function displayScore() {
    const container = document.getElementById('quiz-container');
    container.innerHTML = `
        <div id="score">Quiz Completed! Your Score: ${score} / ${questionCount}</div>
    `;
}
